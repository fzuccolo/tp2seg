...
def validate_case(case_dir: Path, standards_dir: Path) -> None:
    caso = load_yaml(case_dir / "caso.yml")
    standard_id = caso.get("estandar")
    if not standard_id:
        raise ValueError(f"{case_dir}: caso.yml debe declarar estandar")

    catalogo, valid_levels = validate_standard(standards_dir / str(standard_id))
    known_controls = set(catalogo["control_id"].astype(str))

    activos = read_csv(case_dir / "activos.csv")
    diagnostico = read_csv(case_dir / "diagnostico.csv")
    proyectos = read_csv(case_dir / "proyectos.csv")
    proyecto_control = read_csv(case_dir / "proyecto_control.csv")
    entrevistas_path = case_dir / "entrevistas.csv"
    ...
    if diagnostico["control_id"].duplicated().any():
        raise ValueError(f"{case_dir}: diagnostico tiene controles duplicados")
    unknown_diag = diag_controls - known_controls
    if unknown_diag:
        raise ValueError(f"{case_dir}: diagnostico referencia controles inexistentes {sorted(unknown_diag)}")
    missing_diag = known_controls - diag_controls
    if missing_diag:
        raise ValueError(f"{case_dir}: diagnostico debe cubrir todos los controles del estandar, faltan {sorted(missing_diag)}")

    levels = set(diagnostico["nivel_madurez"].astype(int))
    invalid_levels = levels - valid_levels
    if invalid_levels:
        raise ValueError(f"{case_dir}: niveles de madurez invalidos {sorted(invalid_levels)}")
    ...
    if unknown_projects:
        raise ValueError(f"{case_dir}: proyecto_control referencia proyectos inexistentes {sorted(unknown_projects)}")

    unknown_controls = set(proyecto_control["control_id"].astype(str)) - known_controls
    if unknown_controls:
        raise ValueError(f"{case_dir}: proyecto_control referencia controles inexistentes {sorted(unknown_controls)}")

    cid_columns = {"confidencialidad", "integridad", "disponibilidad"}
    if cid_columns <= set(activos.columns):
        cid_frame = activos[list(cid_columns | {"activo_id", "criticidad_cid"})].copy()
        for column in cid_columns | {"criticidad_cid"}:
            cid_frame[column] = pd.to_numeric(cid_frame[column], errors="coerce")
        cid_frame = cid_frame.dropna(subset=list(cid_columns))
        if not cid_frame.empty:
            for column in cid_columns:
                if not cid_frame[column].between(1, 5).all():
                    raise ValueError(f"{case_dir}: {column} debe estar en rango 1..5 cuando esta informado")
            expected_criticality = cid_frame[list(cid_columns)].max(axis=1).astype(int)
            actual_criticality = cid_frame["criticidad_cid"].astype(int)
            invalid_assets = cid_frame.loc[actual_criticality != expected_criticality, "activo_id"].astype(str).tolist()
            if invalid_assets:
                raise ValueError(f"{case_dir}: criticidad_cid debe ser max(C,I,D) en activos con CID completo {invalid_assets}")
    ...
    for column in ["hallazgo", "observaciones"]:
        if column in weak.columns and weak[column].fillna("").astype(str).str.strip().eq("").any():
            raise ValueError(f"{case_dir}: controles debiles deben tener {column}")
    ...
    if weak_without_project:
        raise ValueError(f"{case_dir}: controles debiles sin proyecto asociado {sorted(weak_without_project)}")
...
def main() -> int:
    ...
    cases_dir = ROOT / "datos" / "casos"

    default_case = config.get("caso_default")
    if not default_case or not (cases_dir / str(default_case)).exists():
        errors.append("caso_default no existe")
    ...
    for case_dir in sorted(path for path in cases_dir.iterdir() if path.is_dir()):
        try:
            validate_case(case_dir, standards_dir)
        except Exception as exc:
